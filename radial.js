(() => {
  ...
  const phrasePositions=[[80,-535,80],[345,-505,135]];
  ...
  for(let step=1;step<=10;step++){const frame=document.createElement("i"),t=step/10;frame.style.transform=`translate3d(${80+(345-80)*t-105}px,${-535+30*t-45}px,${80+55*t}px) scale(${.2+.8*t})`;phraseBridge.append(frame)}
  ...
  const landItems=[
    {kind:"lead",x:610,y:-545,z:40,label:"METHOD / LAND ART",title:"LAND ART CAN BE USED LIKE A LENS?",text:"My primary reference is Land Art. I aim to investigate the web through the same conceptual lens with which Land Art artists engaged the landscape."},
    {kind:"image",x:845,y:-425,z:135,src:"assets/lightning-field.png",label:"THE LIGHTNING FIELD",credit:"W. DE MARIA"},
    {kind:"text",x:595,y:-330,z:-35,label:"PROCESS / 01",text:"The artwork is not the landscape itself, but the process of revealing the forces that continuously generate its morphology."},
    {kind:"text",x:825,y:-205,z:80,label:"AUTONOMY / 02",text:"Its existence is autonomous; the artist merely sets the forces in motion."},
    {kind:"image wide",x:575,y:-90,z:155,src:"assets/epigenetic-landscape.png",label:"EPIGENETIC LANDSCAPE",credit:"C. WADDINGTON"},
    {kind:"text large",x:830,y:65,z:-20,label:"LATENT POTENTIAL / 03",text:"The landscape is not designed but generated. Each input produces a new morphology. The epigenetic landscape reveals what already exists as latent potential, but has yet to be expressed."}
  ];
  landItems.forEach((item,index)=>{
    const panel=document.createElement("article");panel.className=`land-panel ${item.kind}`;
  ...
  for(let step=1;step<=8;step++){const frame=document.createElement("i"),t=step/8;frame.style.transform=`translate3d(${900+215*t-90}px,${340+145*t-52}px,${65+80*t}px) scale(${.2+.8*t})`;classificationTrail.append(frame)}
  ...
  const mappingItems=[
    {type:"question",x:-875,y:325,z:80,label:"REPRESENTATION / ECOSYSTEM",title:"HOW CAN WE REPRESENT THE ECOSYSTEM?"},
    {type:"image",x:-1080,y:485,z:155,label:"CARTOGRAPHIC OVERLAY",src:"assets/ecosystem-overlay.png"},
    {type:"image",x:-815,y:535,z:-40,label:"TEMPORAL MAPPING / PLANTING CALENDAR",src:"assets/planting-calendar.png"},
    {type:"image portrait",x:-555,y:430,z:135,label:"MAPPING MATRIX / ECOLOGICAL RELATIONS",src:"assets/mapping-matrix.png"},
    {type:"reference",x:-610,y:650,z:-110,label:"REFERENCE / LANDSCAPE",title:"TAKING MEASURES ACROSS THE AMERICAN LANDSCAPE",text:"James Corner + Alex S. MacLean"}
  ];
  ...
  const powerQuestion=document.createElement("div");powerQuestion.className="power-question";powerQuestion.style.transform="translate3d(-120px,545px,-180px)";powerQuestion.innerHTML="<small>POWER / SYSTEMS / 001</small><strong>HOW ARE THEY RELATED TO THE GENERATION OF POWER?</strong>";powerCluster.append(powerQuestion);
  const powerImage=document.createElement("button");powerImage.className="power-image";powerImage.style.transform="translate3d(165px,595px,35px)";powerImage.innerHTML='<img src="assets/calculating-empires.png" alt="Calculating Empires di Kate Crawford e Vladan Joler"><span><small>CALCULATING EMPIRES</small><b>KATE CRAWFORD + VLADAN JOLER / 2023 &nearr;</b></span>';powerCluster.append(powerImage);
  ...
  const references=[
    {title:"The Stack",author:"Benjamin H. Bratton",src:"assets/the-stack.png",pos:[-355,-415,-520]},
    {title:"Hyperobjects",author:"Timothy Morton",src:"assets/hyperobjects.png",pos:[-110,-470,-430]},
    {title:"Environments of Planetary Urbanization",author:"Neil Brenner, Swarnabh Ghosh, Nikos Katsikis",src:"assets/planetary-urbanization.png",pos:[145,-415,-550]}
  ];
  const referencesTitle=document.createElement("div");referencesTitle.className="references-title";referencesTitle.style.transform="translate3d(20px,-270px,-710px)";referencesTitle.innerHTML="<small>FOUNDATION / 007</small><strong>THEORETICAL GROUND</strong><span>THREE CORE REFERENCES</span>";referencesCluster.append(referencesTitle);
  ...
  const views={
    a:{rx:-2,ry:-3,z:1.18,x:0,y:0},
    b:{rx:4,ry:-8,z:.92,x:-280,y:-400},
    c:{rx:4,ry:8,z:.82,x:535,y:-260},
    d:{rx:-4,ry:5,z:.94,x:420,y:250},
    e:{rx:-5,ry:-7,z:.8,x:-400,y:270},
    f:{rx:-28,ry:-6,z:.72,x:90,y:450}
  };
  ...
  viewport.onwheel=e=>{e.preventDefault();state.tz=Math.max(.42,Math.min(1.5,state.tz-e.deltaY*.0008))};
  nav.forEach(b=>b.onclick=()=>{const view=views[b.dataset.view];state.trx=view.rx;state.try=view.ry;state.tz=view.z;state.tox=view.x;state.toy=view.y;intro.style.opacity=".12";detail.classList.remove("open");about.classList.remove("open");nav.forEach(x=>x.classList.toggle("active",x===b))});
  ...
  onkeydown=e=>{if(e.key==="ArrowLeft")state.try-=8;if(e.key==="ArrowRight")state.try+=8;if(e.key==="ArrowUp")state.trx+=6;if(e.key==="ArrowDown")state.trx-=6;if(e.key==="+"||e.key==="=")state.tz=Math.min(1.5,state.tz+.1);if(e.key==="-")state.tz=Math.max(.42,state.tz-.1);if(e.key==="Escape"){detail.classList.remove("open");about.classList.remove("open")}};
